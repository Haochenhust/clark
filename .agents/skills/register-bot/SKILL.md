...
**You (Codex) automate what you can — writing `.env`, verifying config. The
...
**Do NOT set `ANTHROPIC_API_KEY`** — clark drives interactive `Codex` on your
Codex subscription; setting it would force per-API-token billing.

### Step 6 — Make sure Codex is logged in

clark runs your local `Codex` CLI interactively inside tmux. Run `Codex` once,
...
Edit `workspace/AGENTS.md` — that file is your bot's standing instructions / persona
...
- [ ] `Codex` is logged in and `tmux` is installed
