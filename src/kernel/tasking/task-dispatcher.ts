...
import { config, createLogger, uuid } from "@/sys";


import { scheduledTasks, tasks } from "./data";

const QUEUE_NAME = "clark:tasks";
...
export class TaskDispatcher {
   ...
   *
   * In-flight tasks are aborted via their AbortController before
   * `worker.close()` is awaited. Without this step, `worker.close()` would
   * hang forever in its internal `while (activeJobs > 0)` loop because
   * interactive Claude sessions only complete when externally aborted —
   * they have no natural termination. The abort propagates through
   * `session.stream({ signal })`, the handler's catch block marks the
   * task as `cancelled`, the finally block removes it from
   * `_runningTasks`, and `activeJobs` decrements so the close loop exits.
   */
  async stop(): Promise<void> {
    for (const [taskId, controller] of this._runningTasks) {
      controller.abort();
      this._logger.info(
        { task_id: taskId },
        "aborted running task on dispatcher shutdown",
      );
    }
    if (this._worker) {
   ...
   * with bunqueue's job scheduler. Called on startup and on SIGUSR1 signal.
   */
  async reloadScheduledTasks(): Promise<void> {
    ...
    for (const row of rows) {
      ...
      try {
        // Normalize delay to at (delay is relative, convert to absolute for reload)
        const at =
          sched.at ??
          (sched.delay !== undefined ? r.created_at + sched.delay : undefined);
        ...
        if (at !== undefined && at > now) {
          ...
          if (!existing) {
            ...
            const scheduleWithJobId = { ...sched, at, _job_id: job.id };
            this._db
              .update(scheduledTasks)
        ...
        } else if (at === undefined && (sched.pattern || sched.every)) {
          await this._queue.upsertJobScheduler(
            r.id,
            {
        ...
        this._logger.error(
          { scheduler_id: r.id, schedule: sched, err },
          "failed to reload scheduled task, skipping",
        );
      }
    }
    if (rows.length > 0) {
  ...
  private async _processJob(job: Job<TaskJobData>): Promise<void> {
    ...
    // .catch() prevents a rejected previous from poisoning the entire session chain
